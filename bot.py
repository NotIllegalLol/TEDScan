...
class TEDDataCollector:
    ...
    def filter_high_value_results(self, notices: List[Dict], min_value_eur: float = 15_000_000) -> List[Dict]:
        """Filter for result contracts >= threshold"""
        self.logger.info(f"🔍 FILTERING: form='result' AND value >= €{min_value_eur:,.0f}")
        self.logger.info(f"Input: {len(notices)} notices")
        ...
        for notice in notices:
            try:
                ...
                # STRICT CHECK: Must be exactly "result"
                if form_type != "result":
                    continue
                
                result_count += 1
                self.logger.debug(f"Result notice: {notice.get('publication-number')}")
                ...
                for lot in lots:
                    ...
                    if tender_value:
                        try:
                            ...
                            tender_cur = lot.get('tender_currency', 'EUR')
                            eur_value = self.convert_to_eur(tender_float, tender_cur)
                            total_eur += eur_value
                ...
                if total_eur > 0:
                    ...
                    self.logger.info(f"  Result w/ value: {pub_num} = €{total_eur:,.0f}")

                if total_eur >= min_value_eur:
                    # Extract HTML link
                    links = notice.get('links', {})
                    html_links = links.get('html', {})
                    url = html_links.get('ENG', html_links.get('SWE', 'N/A')) if html_links else 'N/A'
                    ...
                    high_value.append({
                        ...
                        "buyer_country": notice.get("buyer-country", ["N/A"])[0] if isinstance(notice.get("buyer-country"), list) else notice.get("buyer-country", "N/A"),
        ...
        self.logger.info(f"📊 Results: {result_count} result-type, {has_value_count} with values, {len(high_value)} >= €{min_value_eur:,.0f}")
        return high_value
